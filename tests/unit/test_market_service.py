...
from multiverse_market.models.entities import User, Item, Universe
...
class TestMarketService:
    ...
    async def test_buy_item_cross_universe_insufficient_balance(
        ...
        """Test item purchase fails when buying from different universe with insufficient balance after exchange rate."""
        # Add a more expensive Mars item
        mars_item = Item(id=3, name="Mars Item", universe_id=2, price=300.0, stock=5)
        item_repo._items[3] = mars_item
        
        # User from Earth (exchange rate 1.0) trying to buy Mars item (exchange rate 2.5)
        purchase = ItemPurchase(
            buyer_id=1,
            item_id=3,
            quantity=2
    ...
    async def test_list_items_with_universe_filter(
        ...
        item2 = Item(id=2, name="Mars Item", universe_id=2, price=200.0, stock=5)
