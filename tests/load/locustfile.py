...
from locust import HttpUser, task, between, events, LoadTestShape, stats
...
class BaseUser(HttpUser):
    ...
    def retry_with_backoff[T](self, func: T) -> T:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                with self.client.get(LoadTestConfig.HEALTH_CHECK, catch_response=True) as health_check:
                ...
                logger.warning(f"Attempt {attempt + 1} failed, waiting {wait_time}s: {str(e)}")
...
class BrowserUser(BaseUser):
    ...
    def list_universes(self) -> None:
        ...
        with self.client.get(LoadTestConfig.UNIVERSES, catch_response=True, name="List Universes") as response:
    ...
    def get_user(self) -> None:
        ...
        with self.client.get(f"{LoadTestConfig.USERS}/{user_id}", catch_response=True, name="Get User") as response:
...
class TraderUser(BaseUser):
    ...
    def get_user_trades(self) -> None:
        ...
        user_id = random.choice(LoadTestConfig.USER_IDS)
        with self.client.get(f"{LoadTestConfig.TRADES}/{user_id}", catch_response=True, name="Get User Trades") as response:
    ...
    def exchange_currency(self) -> None:
        ...
        def do_exchange() -> ResponseContextManager | None:
            ...
            with self.client.get(f"{LoadTestConfig.USERS}/{user_id}", catch_response=True, name="Get User Balance") as response:
                if not self.handle_response(response, "get_user_balance"):
                    return None
                user_data = response.json()
                self.session.user_balances[user_id] = user_data["balance"]
            
            if user_id not in self.session.user_balances or self.session.user_balances[user_id] <= 0:
                return None
            
            max_amount = self.session.user_balances[user_id] * 0.5
            from_universe = random.choice(LoadTestConfig.UNIVERSE_IDS)
            to_universe = random.choice([u for u in LoadTestConfig.UNIVERSE_IDS if u != from_universe])
            ...
            with self.client.post(LoadTestConfig.EXCHANGE, json=exchange_data, catch_response=True, name="Exchange Currency") as response:
    ...
    def buy_item(self) -> None:
        ...
        def do_purchase() -> ResponseContextManager | None:
            ...
            # Update user balance
            with self.client.get(f"{LoadTestConfig.USERS}/{buyer_id}", catch_response=True, name="Get Buyer Balance") as response:
            ...
            # Get available items
            with self.client.get(LoadTestConfig.ITEMS, catch_response=True, name="List Available Items") as response:
            ...
            available_items = [
                (item_id, data) for item_id, data in self.session.item_stocks.items()
                if data.stock > 0
            ]
            ...
            with self.client.post(LoadTestConfig.BUY, json=purchase_data, catch_response=True, name="Buy Item") as response:
